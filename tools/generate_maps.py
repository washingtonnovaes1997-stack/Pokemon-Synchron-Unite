...
def carve_path(grid,x0,y0,x1,y1,v):
    ...
    for x in range(x0, x1 + sx, sx): carve_rect(grid,x-1,y0-1,x+2,y0+2,v)
    sy = 1 if y1 >= y0 else -1
    for y in range(y0, y1 + sy, sy): carve_rect(grid,x1-1,y-1,x1+2,y+2,v)
...
def generate_city(name,w,h,biome,rules,gym=False):
    ...
    g=[[ground for _ in range(w)] for _ in range(h)]; cx,cy=w//2,h//2
    ...
    for kind,x,y in anchors:
        x=max(3,min(w-4,x)); y=max(3,min(h-4,y)); carve_path(g,cx,cy,x,y,path)
        # Feature anchors represent interaction/door tiles and must remain traversable.
        g[y][x]=path; features.append({"kind":kind,"x":x,"y":y,"visual_marker":codes["landmark"]})

    entrances=[{"direction":"north","x":cx,"y":2},{"direction":"south","x":cx,"y":h-3},{"direction":"west","x":2,"y":cy},{"direction":"east","x":w-3,"y":cy}]
    for e in entrances: carve_path(g,cx,cy,e["x"],e["y"],path); g[e["y"]][e["x"]]=path
    npc_slots=[]
    for i in range(8):
        x=max(3,min(w-4,cx+(i%4)*2-3)); y=max(3,min(h-4,cy+(i//4)*2+4)); carve_path(g,cx,cy,x,y,path); npc_slots.append({"id":i+1,"x":x,"y":y})
...
def main():
    ...
    gyms={g["city"] for g in region["gyms"]}; OUT.mkdir(parents=True,exist_ok=True); maps=[]
